...
Widget _buildEmotionalWellbeing(ProgressViewModel viewModel) {
  ...
  Widget _buildFocusDistribution(ProgressDataModel data, ProgressViewModel viewModel) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          item.label == "Descanso" ? "${item.value}h" : "${item.value} ", 
...
Widget _buildProductivitySection(ProgressDataModel data, ProgressViewModel viewModel) { // 👈 Adicione a viewModel aqui
    const Color colorAtribuidas = Color(0xFFD1D5DB);
    const Color colorConcluidas = Color(0xFF4F46E5);
    
    // ✅ Título Dinâmico
    String tituloSessao = viewModel.isMensal ? "Produtividade Mensal" : "Produtividade Semanal";

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(tituloSessao, // 👈 Agora muda conforme o filtro
            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textColor)),
        const SizedBox(height: 16),
        Container(
          padding: const EdgeInsets.all(16),
  ...
  BarChartData(
      ...
      bottomTitles: AxisTitles(
        sideTitles: SideTitles(
          ...
          getTitlesWidget: (value, meta) {
            ...
            if (index < 0 || index >= data.weeklyProductivity.length) {
    ...
    barGroups: data.weeklyProductivity.asMap().entries.map((entry) {
      ...
      return BarChartGroupData(
        ...
        barRods: [
          BarChartRodData(
            toY: item.attributed.toDouble() == 0 ? 0.1 : item.attributed.toDouble(),
            color: colorAtribuidas,
            width: viewModel.isMensal ? 22 : 16,
            borderRadius: BorderRadius.circular(4),
          ),
          BarChartRodData(
            toY: item.value.toDouble() == 0 ? 0.1 : item.value.toDouble(),
            color: colorConcluidas,
            width: viewModel.isMensal ? 22 : 16,
            borderRadius: BorderRadius.circular(4),
          ),
        ],
        showingTooltipIndicators: [0, 1], // Mostra os números em cima
      );
    }).toList(),
  ),
),
                  ...
                  children: [
                    ...
                    ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                        ...
                        valueColor: const AlwaysStoppedAnimation<Color>(colorConcluidas),
                        minHeight: 12,
                      ),
                    ),
                  ],
...
double _calcularMaxY(List<BarDataModel> dados) {
  ...
  Widget _buildBottlenecksSection(ProgressDataModel data, ProgressViewModel viewModel) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          _buildCustomProgressBar(item.value / 3, const Color(0xFFFF7A00)), 
                        ],
                      ),
                    ),
                  ],
...
Widget _buildRestSection(ProgressDataModel data, ProgressViewModel viewModel) {
  ...
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text("Descanso Real na Semana", 
          style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textColor)), // Reduzi de 18 para 16
      const SizedBox(height: 16),
      Container(
        ...
        child: Column(
          children: [
            Row(
              children: [
                      ...
                      children: [
                        PieChart(
                          PieChartData(
                            ...
                            sections: [
                   PieChartSectionData(color: colorNenhum, value: stats['Nenhum'] ?? 0, showTitle: false, radius: 25),
                             PieChartSectionData(color: colorMais2h, value: stats['Mais de 2 horas'] ?? 0, showTitle: false, radius: 25),
                                PieChartSectionData(color: color1a2h, value: stats['Entre 1 e 2 horas'] ?? 0, showTitle: false, radius: 25),
                   PieChartSectionData(color: color30min, value: stats['30 minutos'] ?? 0, showTitle: false, radius: 25),
                            ],
                          ),
                        ),
                            const Center(child: Icon(Icons.access_time_outlined, color: Colors.black, size: 40)),
                      ],
                    ),
                  ),
                ),
                ...
                Expanded(
                  ...
                  child: Column(
                    children: [
                      const Text("Mais Frequente", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500)),
                   Text(frequente, style: const TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: color30min)),
                    ],
                  ),
                ),
              ],
            ...
            _buildRestLegendItem(colorNenhum, "Nenhum", "${stats['Nenhum']?.round()}%"),
            _buildRestLegendItem(color30min, "30 min", "${stats['30 minutos']?.round()}%"),
            _buildRestLegendItem(color1a2h, "1-2 horas", "${stats['Entre 1 e 2 horas']?.round()}%"),
            _buildRestLegendItem(colorMais2h, "+2 horas", "${stats['Mais de 2 horas']?.round()}%"),
          ],
        ),
      ),
    ],
  ...
  return Padding(
    ...
    child: Row(
      children: [
        ...
        Expanded(child: Text(label, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w400))),
        Text(percent, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w500)),
      ],
    ),
  );
}
